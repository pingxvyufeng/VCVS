    ...
    def train(self, output_dir='EX-0'):
        ...
        for epoch in range(self.hps.epochs_max):
            ...
            for i, key in enumerate(train_keys):
                ...
                seq_1 = dataset['features'][...]
                seq_2 = dataset['RGB_feature'][...]
                seq_3 = dataset['voice_feature'][...]
                seq_1 = torch.from_numpy(seq_1).unsqueeze(0)
                seq_2 = torch.from_numpy(seq_2).unsqueeze(0)
                seq_3 = torch.from_numpy(seq_3).unsqueeze(0)
                ...
                if self.hps.use_cuda:
                    seq_1, seq_2, seq_3, target = seq_1.float().cuda(),seq_2.float().cuda(), seq_3.float().cuda(), target.float().cuda()

                seq_len = seq_1.shape[1]
                y, _ = self.model([seq_1,seq_2,seq_3],seq_len)
                loss_att = 0
    ...
    def eval(self, keys, results_filename=None):
        ...
        with torch.no_grad():
            for i, key in enumerate(keys):
                ...
                seq_1 = data['features'][...]
                seq_2 = data['RGB_feature'][...]
                seq_3 = data['voice_feature'][...]
                seq_1 = torch.from_numpy(seq_1).unsqueeze(0)
                seq_2 = torch.from_numpy(seq_2).unsqueeze(0)
                seq_3 = torch.from_numpy(seq_3).unsqueeze(0)

                if self.hps.use_cuda:
                    seq_1, seq_2, seq_3 = seq_1.float().cuda(), seq_2.float().cuda(),seq_3.float().cuda()

                y, att_vec = self.model([seq_1,seq_2,seq_3], seq_1.shape[1])
                #print (y)
    ...
    for split_filename in hps.splits:
       ...
       # print(n_folds)
        for split_id in range(n_folds):
            ...
            #print(log_file)
            #log_file1 = os.path.join(hps.output_dir,'models')
