...
from margin_loss import CapsuleLoss
import sys
...
def draw(loss_list):
    plt.plot(loss_list, label='F1_Score')
    plt.legend()
    plt.show()

def draw_bar(import_score,machine_summary):
    color={0:'blue',1:'red'}
    index = np.arange(1,len(machine_summary)+1)
    machine_index = np.where(machine_summary==1)
    plt.bar(index, import_score, alpha=1, width=0.8, color=[color[1] if i in machine_index else color[0] for i in index])
    plt.show()
...
#                visFeaturesSize = self.feat_input["feature_size"]
                ...
                if(self.params_obj.sample_technique=="sub"):
                    ...
                    for a in range(len(seq)):
                        seq_temp = seq[a]
                        shot_len = len(cps)
                        re_shot = np.zeros([130, 10, 1024])  # 初始化一个镜头大小的numpy
                        counter = -1
                        target_pre = dataset['gtscore'][...]
                        target = np.zeros(shot_len)
                        for i in range(shot_len - 1):
                            if i == 0:
                                shot_left = 0
                                shot_right = cps[0]
                                if shot_right - shot_left > 10:
                                    sample_frame = random.sample(np.arange(shot_left, shot_right).tolist(), 10)
                                    shot = seq_temp[sample_frame, :]
                                    # target_temp = target_pre.take(sample_frame, axis=0).mean()
                                    target_temp = target_pre[sample_frame].mean()
                                    counter += 1
                                else:
                                    shot_temp = seq_temp[shot_left:shot_right, :]
                                    shot = shot_temp.copy()
                                    shot.resize((10, 1024), refcheck=False)
                                    target_temp = target_pre[shot_left:shot_right].mean()
                                    counter += 1
                            else:
                                shot_left = cps[i] + 1
                                shot_right = cps[i + 1]
                                if shot_left == shot_right:
                                    shot_temp = seq_temp[shot_left, :]
                                    shot = shot_temp.copy()
                                    shot.resize((10, 1024), refcheck=False)
                                    target_temp = target_pre[shot_left]
                                    counter += 1

                                elif shot_right - shot_left > 10:
                                    sample_frame = random.sample(np.arange(shot_left, shot_right).tolist(), 10)
                                    shot = seq_temp[sample_frame, :]
                                    # target_temp = target_pre.take(sample_frame, axis=0).mean()
                                    target_temp = target_pre[sample_frame].mean()
                            ...
                            re_shot[counter, :, :] = shot
                            target[counter] = target_temp
                        seq[a]=re_shot
                ...
                if(self.params_obj.fusion_technique=="early"):
                    ...
                    if self.params_obj.use_cuda:
                        ...
                        for a in range(len(seq)):
                            seq_t = seq[a]
                            seq_t = torch.from_numpy(seq_t).unsqueeze(0)
                            seq_t = seq_t.float().cuda()
                            seq[a] = seq_t
                    seq_len = seq1.shape[1]
                    if(self.params_obj.fusion_technique=="inter"):
                        y,_ = self.model(seq,shot_len) # for three source of feature Xo, Xr, Xf
...
#                visFeaturesSize = self.feat_input["feature_size"]
                ...
                if(self.params_obj.fusion_technique=="early"):
                    ...
                    if self.params_obj.use_cuda:
                        criterion = criterion.cuda()
                    if(self.params_obj.fusion_technique=="inter"):
                        y, att_vec = self.model(seq, shot_len)
                    else:
    ...
    def eval_summary(self, machine_summary_activations, test_keys, results_filename=None, metric='tvsum', att_vecs=None):
        ...
        for key_idx, key in enumerate(test_keys):
            ...
            machine_summary = generate_summary(seq_len,probs, cps,cps_o,num_frames, nfps, positions)
            fm, _, _,importance_score = evaluate_summary(machine_summary, user_summary, eval_metric)
            draw_bar(importance_score,machine_summary)
            fms.append(fm)
